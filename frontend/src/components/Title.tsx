    ...
    const resetConversation = async () => {
        ...
        await axios.get("http://localhost:8000/reset").then((res) => {
            if (res.status == 200) {
                ...
                console.error("There was an error with the backend API Request.");
            }
        })
        .catch((err) => {
  ...
  return (
    <div className="flex justify-between items-center w-full p-4 bg-gray-900 text-white font-bold shadow">
        <div className="italic">Rachel</div>
            <button onClick={resetConversation} className={"transistion-all duration-300 text-blue-300 hover:text-pink-500 " + 
